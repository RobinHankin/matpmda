...
The Student t test (and the associated Student t distribution) has a
complicated mathematical structure and is difficult to work with.
...
which are documented under ```?t.test```.  I give a number of exaples
below.
...
*Quesion:* is there any evidence to suggest that these apples differ
...
is 6.7.  It is expected that the pH level will fall as a result of
...
* step 2.  Decide whether a one-sided or two-sided test.  Because 
as expect a fall in pH levels, we use a one-sided test.
...
Then we can use R to perform the t-test of the null that the fitness level before is equal to the fitness level after:
