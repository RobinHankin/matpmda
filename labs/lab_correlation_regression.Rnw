...
\newcommand{\courseCode}[0]{STAT500}
...
\item Is there a relationship between qualification level and networth?
\end{itemize}
...
There is a statistically significant difference between the mean income level for people with different qualifications.
...
There is not a statistically significant difference between the mean income level for males and females (p value is 0.4238 > 0.05).
...
%\item Download the global indicators dataset from assignment 2. Investigate the relationship between variables of your choice using correlation and regression.
%\end{enumerate}
%
\end{enumerate}
