...
```{r onesidedtest,fig.cap="pvalue for a one-sided test", echo=FALSE,message=FALSE,warning=FALSE}
...
legend("topright",col=c("red","black"),lwd=c(11,1),legend=c("p-value","Null distribution"))
```

```{r twosidedtest,fig.cap="pvalue for a two-sided test", echo=FALSE,message=FALSE,warning=FALSE}
x <- seq(from=-4,to=4,by=0.05)
...
legend("topright",col=c("red","black"),lwd=c(11,1),legend=c("pvalue","Null distribution"))
```
...
* _Step 6_.  Compare the pvalue of about 3.7\% with the 5\% standard
  value.  Thi s is less than 5\% So we *reject* the null.
...
* *STEP 1*. State a null hypothesis.  Here we would say that the null is
