...
To get a 99\% confidence interval we have exactly the same
construction except we need to use ```qnorm(0.995)```:
...
There are only six datapoints, so the $Z$-score method is inapplicable.  The ```t.test()``` in R gives:
...
for(i in 1:70){
  ...
  segments(x0=jj[1],x1=jj[2],y0=i,lwd=3,col=col)
}
abline(v=0)
```