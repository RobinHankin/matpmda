...
* Grams of protein in milk from different locations
...
experiment).  STAT500 is not going to focus on the collection of data;
other AUT papers give it more attention.
...
There is a list of commonly-used commands at the end of the manual
that may be useful.
...
(see the Appendix for an explanation of the $\sum$ notation).  It is
easy to calculate the mean in R using the ```mean()``` function:
...
plot(x,dchisq(x,df=3),lwd=5,type='l',axes=F,xlab='',ylab='',main='negative skew')
...
plot(x,dchisq(10-x,df=3),lwd=5,type='l',axes=FALSE,xlab='',ylab='',main='positive skew')
...
In the above we see a left-skewed distribution, a symmetrical
...
a measure of the engine power) on the y axis.  The tilde symbol
