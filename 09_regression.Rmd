...
```{r residuals,fig.cap="Residuals on a regression", out.width='100%',echo=FALSE,message=FALSE,warning=FALSE}
...
```{r twoplots,fig.cap="two plots with different strengths of association",out.width='100%',echo=FALSE}
x <- 1:100
...
```{r fourplots,fig.cap="Different correlation coefficients",out.width='100%',echo=FALSE}
x <- 1:100
y1 <- +x+rnorm(100)
y2 <- +x+rnorm(100)*37
y3 <- -x+rnorm(100)*37
...
```{r LINE,fig.cap="Contraindications for linear regression.   Top left, non-linear response; top right, non-independent errors; lower left, non-Gaussian errors; lower right, non-equal variances", out.width='100%',echo=FALSE}
n <- 60
...
```{r, plot3d, fig.cap="Visual representation of multivariate linear regression, taken from the plot3D package.  Variable `mpg` is shown on the vertical axis; variables `wt` and `disp` on the two horizontal axes.  The regression itself is a plane, rather than a line", out.width='100%',echo=FALSE}
