...
Figure \@ref(fig:manynorm) shows
some normal distributions with varying mean values (the curve moves
left and right) and widths (the curve becomes wider or narrower).
...
In the above, we specified the mean and standard deviation explicitly,
but naming them is optional.  We could have said
```rnorm(20,10,0.03)``` and obtained the same result.  Graphical plots
are always informative; see figure \@ref(fig:normplot) for an example.

```{r normplot,fig.cap="1000 Random observations from the standard normal distribution.  See how most of the observations are between $-2$ and $+2$ but there are occasional outliers", echo=TRUE,message=FALSE,warning=FALSE}
plot(rnorm(1000))
```

It is possible to produce a histogram using ```hist()``` as in figure
\@ref(fig:normhist).

```{r normhist,fig.cap="Histogram of random observations from the standard normal distribution.  See how most of the observations are between $-2$ and $+2$ but there are occasional outliers", echo=TRUE,message=FALSE,warning=FALSE}
...
```{r normcumulativeplot, fig.cap="The cumulative density function is the probability that a random variable falls to the left of a particular value, here 1.3", echo=TRUE,message=FALSE,warning=FALSE}
...
$X$ is less than a particular value, as in figure
\@ref(fig:normcumulativeplot).  This is calculated by the
```pnorm()``` command in R.  This shows a value of 1.3 and the
probability that $X\leq 1.3$ is given by the gray area.  To give a
numerical value we can say
