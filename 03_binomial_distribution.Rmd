...
experiment with two possible outcomes, conventionally named
``success'' and ``failure''.
...
The R output is coded in terms of ``0'' for a failure and ``1'' for a
success (in the ```rbinom()``` command, the _arguments_ are the three
numbers 100,1,0.5.  The 100 means to simulate 100 trials, the ``1''
means to toss the coin once, and the 0.5 gives the probability of
success).  See how about half the trials are successes.
...
## Increaing the number of trials
...
A random distribution (here the binomial) has four R functions
...
million undergraduates tossing a coin 5 times and recording the number
of heads:
...
So, out of a million observations we have exactly 309665 students with
two heads, a proportion of 0.309665, not too far from the exact value
...
* mean value of the binomial distribution  $E[X] = np$
* variance of the binomial distribution  $\text{Var}[X] = np(1-p)$
* standard deviation of the binomial distribution $\text{Std}[X] =\sqrt{np(1-p)}$.

### Example

We can verify these formulas directly using random sampling.
